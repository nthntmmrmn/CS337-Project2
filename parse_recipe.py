...
def parse_ingredients_helper(ing):
    ...
    #      if r else ['', get_type_of_ingredient(re.search('[0-9\u00BC-\u00BE\u2150-\u215E]+(.*)', ing).group(1))]]
...
def get_type_of_ingredient(text):
    ...
    if len(tokens)>1:
        ...
        print(first_jj, last_nn)
        # stop = next((i for i,v in enumerate(tokens) if v[1] in desc_pos and i != last_nn), last_nn)
        # print(stop)
        if last_nn + 1 == last_nns:
            if first_jj:
                ...
                typ = tokens[last_nn][0]
            # desc = ' '.join([v[0] for v in tokens[stop:last_nn]] + [tokens[last_nns][0]])
            # typ = tokens[last_nn] #' '.join([v[0] for v in tokens[last_nn]])
            # desc = ' '.join([v[0] for v in tokens[:stop]] + [tokens[last_nns][0]])
            # typ = ' '.join([v[0] for v in tokens[stop:last_nns]])
            ...
            if first_jj:
                ...
                typ = tokens[last_nn][0]
            # desc = ' '.join([v[0] for v in tokens[:stop]])
            # typ = ' '.join([v[0] for v in tokens[stop:]])
    else: desc = ''
    ...
    #         re.sub(r'\s+([,:;-])', r'\1', rest)] if not flip else \
    #             [re.sub(r'\s+([,:;-])', r'\1', rest),
    #              re.sub(r'\s+([,:;-])', r'\1', type_ing).strip(string.punctuation)]
